    ...
    def getREGEXPATH(self, filename):
        ...
        try:
            for REGEX_PATH in self.SECTIONS['REGEX_PATH'] :
                logger.logger.info(f'getREGEXPATH :::: filename=[{filename}] REGEX_PATH=[{REGEX_PATH}]')
                flags = 0
                pattern = REGEX_PATH
                ...
                logger.logger.info(f'pattern i :::: filename=[{filename}] pattern=[{pattern}] flags=[{flags}] ')
                if re.search(pattern, filename, flags=flags):
                    download_path = os.path.join(self.SECTIONS['REGEX_PATH'][REGEX_PATH])
